...
```json
{
  "description": "Результат выполнения. Может быть любым значением (any)."
}
```
...
{
  "type": "object",
  "description": "Пустой объект. Успешный вызов означает, что нативное окно предпросмотра PDF было успешно инициализировано и открыто."
}
```
...
{
  "type": "boolean",
  "description": "Результат авторизации (true — успешно, false — ошибка или отмена)."
}
```
...
{
  "type": "object",
  "description": "Пустой объект. Успешный ответ (HTTP 200) означает, что запись была успешно произведена."
}
```
...
| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `permissions` | `array` | *См. подробное описание ниже* | **Да** |

---

#### Подробные описания полей

##### Поле: `permissions` (array)
...
```json
{
  ...
  "text": "string",
  "subject": "string",
  "sharePositionOrigin": "array"
}
```

| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `paths` | `array` | Список локальных путей к файлам, которые нужно расшарить. | **Да** |
...
| `sharePositionOrigin` | `array` | Координаты для iPad `[left, top, width, height]`. Определяют точку, из которой «вылетит» системное меню шеринга. | Нет |
...
| `sharePositionOrigin` | `array` | *См. подробное описание ниже* | Нет |

---

#### Подробные описания полей

##### Поле: `sharePositionOrigin` (array)
...
| `sharePositionOrigin` | `array` | Координаты для iPad `[left, top, width, height]`. Нужны, чтобы стрелочка всплывающего окна (popover) указывала на правильный элемент интерфейса. | Нет |
...
{
  "type": "boolean",
  "description": "Результат операции: `true` — системное приложение для звонков успешно открыто, `false` — возникла ошибка (например, на устройстве нет телефонного модуля)."
}
```
...
{
  "type": "object",
  "description": "Пустой объект. Успешное выполнение (HTTP 200) подтверждает, что команда на скрытие лоадера была получена хост-приложением."
}
```
...
{
  "type": "object",
  "description": "Пустой объект. Успешный ответ подтверждает, что нижняя навигационная панель была скрыта."
}
```
...
{
  "type": "object",
  "description": "Пустой объект. Успешный вызов (HTTP 200) подтверждает, что команда на отображение нижней навигационной панели (Tab Bar) была исполнена хост-приложением."
}
```
