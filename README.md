...
| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `product` | `string` | Уникальный идентификатор микрофронта (продукта), которому адресовано сообщение. Например: `sbp_microfront_ui`. | **Да** |
| `eventCode` | `string` | *См. подробное описание ниже* | Нет |

**Подробные описания полей:**

> **Поле: `eventCode`** (string)
>
Код конкретного события. 

| Параметр | Обязательность | Логика фильтрации |
| :--- | :--- | :--- |
| `product` | Да | Только сообщения для данного продукта. |
| `eventCode` | Нет | Если передан — фильтрация по коду. Если не передан — получение **всех** событий продукта. |
