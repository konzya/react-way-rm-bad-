...
- [waiter.show](#serviceswaitershow)
...
> Данные, возвращаемые веб-модулем после завершения работы.

| Тип | Описание |
| :--- | :--- |
| `any` | Результат может быть любого типа (строка, число, объект или массив). Структура данных определяется вызываемым модулем и передается обратно при его закрытии. |
...
> Результат выполнения. Может быть любым значением (any).
...
> Пустой объект. Успешный вызов означает, что нативное окно предпросмотра PDF было успешно инициализировано и открыто.
...
> Результат авторизации (true — успешно, false — ошибка или отмена).
...
> Тип доступной биометрии на устройстве.

| Значение | Тип | Описание |
|---|---|---|
| `none` | Нет | Биометрическая аутентификация недоступна или не настроена на устройстве. |
| `fingerprint` | Отпечаток | Доступен сканер отпечатков пальцев (TouchID и аналоги). |
| `face` | Лицо | Доступно распознавание лица (FaceID и аналоги). |
| `iris` | Радужка | Доступно сканирование радужной оболочки глаза. |
...
*Схема: `BiometricsIsBiometricsEnabledResponse`*

**Возвращаемое значение:** `boolean`

> Текущий статус биометрической авторизации в приложении.

| Значение | Описание |
| :--- | :--- |
| `true` | Биометрия (FaceID/TouchID/Fingerprint) включена и может быть использована. |
| `false` | Биометрия отключена в настройках приложения или не поддерживается устройством. |
...
```json
{
  "isLimitedAccess": "boolean - required",
...
| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `isLimitedAccess` | `boolean` | Флаг частичного доступа к контактной книге (актуально для iOS 18 и выше). | **Да** |
| `contacts` | `array[object]` | *См. подробное описание ниже* | **Да** |

**Подробные описания полей:**

> **Поле: `contacts`** (array[object])
>
Массив объектов с данными контактов.

| Свойство | Тип | Описание |
| :--- | :--- | :--- |
| `name` | `string` | Полное имя контакта. |
| `phone` | `string` | Номер телефона (обычно в формате без пробелов). |
| `img` | `string` | Фотография контакта (строка в формате Base64 или путь к локальному кэшу). |
...
```json
{
  ...
  "osVersionNumeric": "number - required",
  "imsi": "string",
  "imei": "string",
  "imeisv": "string",
  "msisdn": "string",
  "spn": "string",
  "osVersion": "string",
  "webViewVersionNumeric": "string",
  "identifierForVendor": "string",
  "osNameVersion": "string",
  "appleWebKitVersionNumeric": "string",
...
| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `model` | `string` | Модель устройства (например, 'iPhone 13' или 'Samsung SM-G991B'). | **Да** |
| `osVersionNumeric` | `number` | Версия ОС в виде числа для удобства сравнения (например, 15.4). | **Да** |
| `imsi` | `string` | Идентификатор абонента SIM-карты (IMSI). | Нет |
| `imei` | `string` | Международный идентификатор мобильного оборудования (IMEI). | Нет |
| `imeisv` | `string` | Версия программного обеспечения IMEI (IMEI SV). | Нет |
| `msisdn` | `string` | Номер телефона, привязанный к SIM-карте (MSISDN). | Нет |
| `spn` | `string` | Service Provider Name — наименование оператора сотовой связи. | Нет |
| `osVersion` | `string` | Версия операционной системы строкой (например, '14.0.1'). | Нет |
| `webViewVersionNumeric` | `string` | Мажорная версия системного WebView (актуально для Android). | Нет |
| `identifierForVendor` | `string` | IDFV (iOS) — уникальный ID устройства для конкретного вендора приложения. | Нет |
| `osNameVersion` | `string` | Комбинированная строка ОС и версии (например, 'Android 12'). | Нет |
| `appleWebKitVersionNumeric` | `string` | Мажорная версия движка AppleWebKit (актуально для iOS). | Нет |
| `deviceFingerprint` | `string` | Цифровой отпечаток устройства для систем безопасности и антифрода. | **Да** |
...
```json
{
  ...
  "fingerprintUsed": "boolean - required"
}
```

| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `id` | `string` | Уникальный идентификатор устройства в системе. | **Да** |
| `platformType` | `string` | Тип ОС: `android` или `ios`. | **Да** |
| `platformVersion` | `string` | Версия операционной системы устройства. | **Да** |
| `model` | `string` | Коммерческое название модели (например, 'iPhone 14 Pro'). | **Да** |
| `manufacturer` | `string` | Заводской производитель устройства. | Нет |
| `nfc` | `boolean` | Наличие и доступность NFC-модуля. | Нет |
| `iccid` | `string` | Интегрированная идентификационная карта (ID SIM-карты). | Нет |
| `msisdn` | `string` | Номер мобильного телефона абонента. | Нет |
| `imsi` | `string` | Международный идентификатор мобильного абонента. | Нет |
| `imei` | `string` | Уникальный серийный номер оборудования (IMEI). | Нет |
| `imeisv` | `string` | Версия программного обеспечения IMEI. | Нет |
| `spn` | `string` | Название оператора сотовой связи (Service Provider Name). | Нет |
| `androidId` | `string` | Уникальный 64-битный идентификатор для Android-устройств. | Нет |
| `identifierForVendor` | `string` | IDFV — уникальный ID для iOS-приложений одного поставщика. | Нет |
| `ipAddress` | `string` | Текущий локальный или внешний IP-адрес. | Нет |
| `authType` | `string` | *См. подробное описание ниже* | **Да** |
| `fingerprintUsed` | `boolean` | Флаг, указывающий, что вход в текущую сессию был выполнен через отпечаток пальца. | **Да** |

**Подробные описания полей:**

> **Поле: `authType`** (string)
>
Способ текущей аутентификации клиента.

| Значение | Описание |
| :--- | :--- |
| `login, sms` | Логин + СМС-код |
| `card, sms` | Номер карты + СМС-код |
| `pin` | Вход по ПИН-коду |
| `fingerprint` | Биометрия (отпечаток) |
| `faceId` | Биометрия (лицо) |
...
| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `value` | `string` | *См. подробное описание ниже* | Нет |

**Подробные описания полей:**

> **Поле: `value`** (string)
>
Считанное значение, привязанное к ключу.

| Ситуация | Возвращаемое значение |
| :--- | :--- |
| **Ключ найден** | Строка с данными (может быть JSON-строкой). |
| **Ключ не найден** | `null` или пустая строка `""`. |
| **Ошибка чтения** | Код ошибки в формате JSON-RPC. |
...
> Пустой объект. Успешный ответ (HTTP 200) означает, что запись была успешно произведена.
...
> Флаг доступности сервиса PayControl на текущем устройстве.

| Значение | Описание |
| :--- | :--- |
| `true` | Сервис установлен, настроен и готов к подписанию операций. |
| `false` | Сервис недоступен (не установлены ключи, устройство не прошло проверку безопасности или ПО отсутствует). |
...
```json
{
  "payControlUserId": "string - required",
  "payControlTransactionId": "string - required",
  "confirmationCode": "string - required",
  "isBiometricIDConfirmation": "boolean - required"
}
```

| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `payControlUserId` | `string` | Уникальный идентификатор пользователя в системе PayControl. | **Да** |
| `payControlTransactionId` | `string` | Уникальный идентификатор транзакции в системе PayControl. | **Да** |
| `confirmationCode` | `string` | Финальный код (подпись), сгенерированный системой для подтверждения операции. | **Да** |
| `isBiometricIDConfirmation` | `boolean` | Указывает, подтвердил ли пользователь подпись через биометрию (true) или вводом ПИН-кода PayControl (false). | **Да** |
...
> Уникальный идентификатор (UID) пользователя в системе PayControl.

| Формат | Описание |
| :--- | :--- |
| **String** | Регистрационный ID пользователя, необходимый для инициализации операций подписания через сервер PayControl. |
| **null / Error** | Если пользователь не зарегистрирован в системе на данном устройстве. |

**Пример ответа:**
```json
"6f8d84a5-1ada-438c-94f6-92574435b302"
```
...
| `notRequested` | Доступ еще не запрашивался у пользователя. |
| `denied` | Доступ отклонен, но система позволяет запросить его снова. |
| `deniedAlways` | Доступ запрещен окончательно (пользователь нажал 'Don't ask again' или запретил в настройках). |
| `restricted` | Доступ ограничен (например, из-за настроек родительского контроля). |
...
> Объект, в котором ключи соответствуют запрошенным функциям, а значения — их новым статусам доступа.

**Возможные значения статуса:**

| Статус | Описание |
|---|---|
| `granted` | Доступ предоставлен пользователем. |
| `notRequested` | Доступ не запрашивался (или запрос был прерван). |
| `denied` | Пользователь отклонил запрос. |
| `deniedAlways` | Пользователь запретил доступ и выбрал «Больше не спрашивать» (требуется переход в настройки). |
| `restricted` | Доступ ограничен на уровне системы (OS). |
...
> Текущий статус разрешений на пуш-уведомления.

| Значение | Описание |
| :--- | :--- |
| `true` | Уведомления разрешены на системном уровне и подписка активна. |
| `false` | Уведомления заблокированы пользователем в настройках ОС или отключены в приложении. |
...
> Статус выполнения команды на изменение состояния подписки.

| Значение | Описание |
| :--- | :--- |
| **success** | Команда принята нативным слоем. Это не гарантирует мгновенную подписку в ОС, а лишь подтверждает старт процесса. |

**Примечание:** Для проверки итогового результата (разрешил ли пользователь уведомления в системном диалоге) используйте метод `push.isPushEnabled`.
...
> Результат сканирования и расшифровки QR-кода.

| Платформа | Поведение |
| :--- | :--- |
| **Cordova** | Возвращает строку с данными, содержащимися в QR-коде (URL, текст, JSON). |
| **Flutter** | Может возвращать пустую строку, если логика обработки (например, переход по ссылке) полностью инкапсулирована в нативном слое. |

**Примечание:** Если пользователь закрыл сканер, не считав код, метод может вернуть ошибку или `null`.

**Пример ответа:**
```json
"https://www.rshb.ru/pay?id=12345"
```
...
> Результат распознавания QR-кода из выбранного графического файла.

| Платформа | Поведение |
| :--- | :--- |
| **Cordova** | Возвращает строку с расшифрованными данными QR-кода. |
| **Flutter** | Может возвращать пустую строку, если логика обработки данных (например, платежа) инкапсулирована в нативном слое и результат сразу уходит в микросервисы. |

**Примечание:** Если файл не содержит валидного QR-кода или пользователь отменил выбор файла, метод вернет ошибку или пустую строку.

**Пример ответа:**
```json
"ST00012|Name=RSHB|PersonalAcc=40702810..."
```
...
> Результат вызова системного диалога «Поделиться».

| Значение | Описание |
| :--- | :--- |
| **success** | Системное окно (Share Sheet) было успешно открыто. |

**Важно:** Статус `success` подтверждает только факт открытия окна выбора приложений. Нативный слой не получает информацию о том, какое именно приложение выбрал пользователь и был ли файл отправлен успешно.
...
> Результат вызова системного окна «Поделиться» для текста.

| Значение | Описание |
| :--- | :--- |
| **success** | Системный диалог успешно открыт. |

**Примечание:** Как и в случае с файлами, статус `success` указывает только на открытие окна. Успешность отправки текста в конкретное приложение (Telegram, SMS и др.) нативным слоем не отслеживается.
...
> Результат вызова системного окна «Поделиться» для ссылки (URL).

| Значение | Описание |
| :--- | :--- |
| **success** | Системный диалог успешно открыт. |

**Примечание:** В отличие от простого текста, при шеринге URL операционная система может попытаться подтянуть превью страницы (заголовок, иконку) перед открытием списка приложений.
...
> Результат проверки СМС-кода.

| Значение | Описание |
| :--- | :--- |
| `true` | Код успешно проверен на стороне сервера, операция подтверждена. |
| `false` | Ошибка проверки (неверный код) или пользователь отменил ввод. |

**Примечание:** В случае истечения времени жизни кода или превышения количества попыток метод обычно возвращает ошибку в формате JSON-RPC.
...
> Результат операции: `true` — системное приложение для звонков успешно открыто, `false` — возникла ошибка (например, на устройстве нет телефонного модуля).
...
> Пустой объект. Успешное выполнение (HTTP 200) подтверждает, что команда на скрытие лоадера была получена хост-приложением.


---

### services.waiter.show

Блокирует интерфейс веб-модуля и отображает нативный индикатор загрузки. 

**Особенности:**
- Для вызова требуется идентификатор модуля: `WebModuleService_Waiter`.
- На Flutter-платформе выбор стиля лоадера через `type` недоступен.
- **Обязательно** вызывайте `waiter.hide` после завершения операции, иначе интерфейс останется заблокированным.

**Тело запроса:**

*Схема: `WaiterShowRequest`*
...
| Поле | Тип | Описание | Обязательное |
| --- | --- | --- | --- |
| `type` | `string` | *См. подробное описание ниже* | Нет |

**Подробные описания полей:**

> **Поле: `type`** (string)
>
Тип анимации лоадера (используется только в Cordova).

| Значение | Платформа | Описание |
|---|---|---|
| `apple` | Cordova | Системный индикатор в стиле iOS (ромашка). |
| `material` | Cordova | Системный индикатор в стиле Android. |
| *любое* | Flutter | **Игнорируется**. Flutter-приложение всегда показывает стандартный лоадер, определенный в теме хоста. |
...
> Пустой объект. Успешный ответ подтверждает, что нижняя навигационная панель была скрыта.
...
> Пустой объект. Успешный вызов (HTTP 200) подтверждает, что команда на отображение нижней навигационной панели (Tab Bar) была исполнена хост-приложением.
